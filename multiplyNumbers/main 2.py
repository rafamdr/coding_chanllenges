...
class Solution:
    ...
    def multiply_str_karatsuba_non_rec(self, number1: str, number2: str) -> str:
        stack_op = []
        stack_op.append([number1, number2])
        ...
        while(len(stack_op)):
            if len(temp_op) == 3:
                ...
                z1 = temp_op[1]
                z2 = temp_op[2]
                res = self._add_multiple_str([
                                (z2 + ('0' * (2 * (m - m_half)))),
                                (self._subtract_multiple_str([z1, z2, z0]) + ('0' * (m - m_half))),
                                z0
                            ]).lstrip('0')
                stack_op.append(res)
            elif isinstance(stack_op[-1], str):
                temp_op.append(stack_op.pop())
            elif isinstance(stack_op[-1], list):
                number1, number2 = stack_op.pop()
                if len(number1) == 1 and len(number2) == 1:
                    return str(int(number1) * int(number2))
                if len(number1) < len(number2):
                    number1 = ('0' * (len(number2) - len(number1))) + number1
                ...
                high1, low1 = number1[:m_half], number1[m_half:]
                high2, low2 = number2[:m_half], number2[m_half:]
                stack_op.append([low1, low2])
                stack_op.append([self._add_str(low1, high1), self._add_str(low2, high2)])
                stack_op.append([high1, high2])
...
def main():
    ...
    for key in examples:
        ...
        print('Karat. NR: Exp = %s  ->  Res = %s' % (key, sol.multiply_str_karatsuba_non_rec(ex[0], ex[1])))
        print()
# ---------------------------------------------------------------------------------------------------------------------
